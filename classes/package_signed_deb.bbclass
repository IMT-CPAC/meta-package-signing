...
# Additional work for signed packages by G. Oliver <go@robosity.com>
#
...
python do_package_deb () {
    ...
    for file in os.listdir(pkgoutdir):
        if len(launched) < max_process:
            p = DebianWritePkgProcess(target=deb_sign_pkg, args=(os.path.join(pkgoutdir, file), d))
            p.start()
            launched.append(p)
        for q in launched:
            # The finished processes are joined when calling is_alive()
...
do_package_deb[vardeps] += "deb_write_pkg deb_sign_pkg"
...
def deb_sign_pkg(file, d):
    import subprocess

    bb.note("*** signing package in %s:" % file)

    subprocess.check_output("PATH=\"%s\" ls -l %s" % (d.getVar("PATH"), file),
                            stderr=subprocess.STDOUT,
                            shell=True)

# Otherwise allarch packages may change depending on override configuration
deb_sign_pkg[vardepsexclude] = "OVERRIDES"

# Indirect references to these vars
do_package_write_signed_deb[vardeps] += "PKGV PKGR PKGV DESCRIPTION SECTION PRIORITY MAINTAINER DPKG_ARCH PN HOMEPAGE"

SSTATETASKS += "do_package_write_signed_deb"
do_package_write_signed_deb[sstate-inputdirs] = "${PKGWRITEDIRDEB}"
do_package_write_signed_deb[sstate-outputdirs] = "${DEPLOY_DIR_DEB}"

python do_package_write_signed_deb_setscene () {
    tmpdir = d.getVar('TMPDIR')
...
addtask do_package_write_signed_deb_setscene

python () {
    if d.getVar('PACKAGES') != '':
        deps = ' dpkg-native:do_populate_sysroot virtual/fakeroot-native:do_populate_sysroot'
        d.appendVarFlag('do_package_write_signed_deb', 'depends', deps)
        d.setVarFlag('do_package_write_signed_deb', 'fakeroot', "1")
}

python do_package_write_signed_deb () {
    bb.build.exec_func("read_subpackage_metadata", d)
    bb.build.exec_func("do_package_deb", d)
}
do_package_write_signed_deb[dirs] = "${PKGWRITEDIRDEB}"
do_package_write_signed_deb[cleandirs] = "${PKGWRITEDIRDEB}"
do_package_write_signed_deb[umask] = "022"
do_package_write_signed_deb[depends] += "${@oe.utils.build_depends_string(d.getVar('PACKAGE_WRITE_DEPS'), 'do_populate_sysroot')}"
addtask package_write_signed_deb after do_packagedata do_package


PACKAGEINDEXDEPS += "dpkg-native:do_populate_sysroot"
PACKAGEINDEXDEPS += "apt-native:do_populate_sysroot"

do_build[recrdeptask] += "do_package_write_signed_deb"
